...
/**
 * Siembra todos los departamentos y municipios de Colombia
 * con códigos DANE oficiales (2 dígitos departamento, 5 dígitos municipio).
 *
 * Fuente: DANE – Divipola 2024 (1.122 municipios, 33 departamentos)
 */
class ColombiaGeographySeeder extends Seeder
{
    public function run(): void
    {
        ...
        $colombia = Country::updateOrCreate(
            ['code' => 'CO'],
            ['name' => 'Colombia', 'phone_code' => '+57', 'is_active' => true]
        );

        // ── Departamentos ────────────────────────────────────────────────
        // Clave: código DANE 2 dígitos (string con cero a la izquierda)
        $departamentos = [
            '05' => 'Antioquia',
            '08' => 'Atlántico',
            '11' => 'Bogotá D.C.',
        ...
        $deptModels = [];
        foreach ($departamentos as $code => $name) {
        ...
        // ── Municipios ───────────────────────────────────────────────────
        // Formato: ['código DANE 5 dígitos', 'Nombre municipio']
        // Los primeros 2 dígitos identifican el departamento.
        $municipios = [
            // ── ANTIOQUIA (05) ──
            ['05001', 'Medellín'],
            ['05002', 'Abejorral'],
        ...
        // Insertar municipios agrupando por departamento (primeros 2 dígitos del código)
        $cityInserts = [];
        $now = now();
        $bogotaId = null;

        foreach ($municipios as [$code, $name]) {
            $deptCode = substr($code, 0, 2);
            if (!isset($deptModels[$deptCode])) {
                continue; // Departamento no encontrado, omitir
            }
            $cityInserts[] = [
                'department_id' => $deptModels[$deptCode]->id,
                'name'          => trim($name),
                'code'          => $code,
                'is_active'     => true,
                'created_at'    => $now,
        ...
        // Insertar en lotes de 200 para eficiencia
        foreach (array_chunk($cityInserts, 200) as $chunk) {
            City::insert($chunk);
        }

        // ── Zonas de Bogotá ─────────────────────────────────────────────
        $bogota = City::where('code', '11001')->first();
        if ($bogota) {
            $zonasBogota = [
                'Norte', 'Sur', 'Centro', 'Oriente', 'Occidente',
                'Usaquén', 'Chapinero', 'Kennedy', 'Suba',
            ];
            foreach ($zonasBogota as $zoneName) {
                Zone::create([
                    'city_id'   => $bogota->id,
                    'name'      => $zoneName,
                    'is_active' => true,
        ...
        $total = count($cityInserts);
        $this->command->info("✓ Colombia sembrada: {$total} municipios en " . count($departamentos) . ' departamentos.');
        if ($bogota) {
            $this->command->info('✓ 9 zonas de Bogotá creadas.');
        }
    }
}
